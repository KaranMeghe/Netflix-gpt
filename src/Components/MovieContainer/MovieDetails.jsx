...
const MovieDetails = () => {
    ...
    return (
        <div className="px-10 pt-20 bg-black h-screen"> {/* Add padding-top to prevent overlap */}
            <Link to="/browse">
                <Button
            ...
            <div className="flex justify-around items-center "> {/* Reduce height */}
                {poster_path && (
                    <img src={`${IMG_CDN_URL}${poster_path}`} alt={`${title} poster`} className="w-96" />
                )}
                <div className="flex flex-col justify-center gap-6 text-white p-10">
                    <h2 className="text-4xl">{title}</h2>
                    <p className="text-xl">{overview}</p>
                    <p className="font-bold text-xl">Release date: {release_date}</p>
